...
class AnalysisMeg:
    def __init__(self):
        ...
        self.linkSub = rospy.Subscriber("link", String, self.callback)
    ...
    def callback(self,msg: String):
        ...
        print(msg.data)
        # 1020.20dm010.30dm200.41dm
        if msg.data[0] == "1":
            x_pos = msg.data[1:9]
            y_pos = msg.data[9:17]
            # datas = msg.data[2: -1].split(",")
            # x_pos = str(float(datas[0])/10000)
            # y_pos = str(float(datas[1])/10000)

            x_now,y_now  = self.maptf(float(x_pos[:-2])*10,float(y_pos[:-2])*10)
            sum = msg.data[17:25]   # todo
        ...
        # elif msg.data[0] == 'M' and msg.data[-1] == 'C':
            ...
            # 20145205123
            #target_x_int = int(msg.data[1:4])
            #target_y_int = int(msg.data[6:9])

            datas = msg.data[2: -1].split(",")
            target_x_int = int(datas[0])//10
            target_y_int = int(datas[1])//10

            map_x,map_y = self.maptf(target_x_int,target_y_int)

            tar_x = "DS24(12,173,'"+(3-len(str(target_x_int//10)))*' '+str(target_x_int)[:-1]+'.'+msg.data[3:5]+"dm',15);"
            tar_y = "DS24(12,215,'"+(3-len(str(target_y_int//10)))*' '+str(target_y_int)[:-1]+'.'+msg.data[8:10]+"dm',15);"
